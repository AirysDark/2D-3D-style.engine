...
namespace Oak
{
    ...
    public class Prototype : DynamicObject, IDictionary<string, object>
    {
        ...
        {
            return (Enumerable() as System.Collections.IEnumerable).GetEnumerator();
        }

        public override string ToString()
        {
            return PrismInfo.Parse(this);
        }
    }
    ...
    [DebuggerNonUserCode]
    public class Prism : DynamicObject
    {
        static Prism()
        {
            Prism.Initialized<Prism>(d => d.AddRedefinableDelegates());
        }

        private bool initialized;

        private static List<KeyValuePair<Type, Func<dynamic, dynamic>>> ExtendHooks =
            new List<KeyValuePair<Type, Func<dynamic, dynamic>>>();

        private static List<KeyValuePair<Type, Action<dynamic>>> MethodHooks =
            new List<KeyValuePair<Type, Action<dynamic>>>();

        private static List<KeyValuePair<Type, Action<dynamic>>> InitializationHooks =
            new List<KeyValuePair<Type, Action<dynamic>>>();

        private static Dictionary<Type, List<MethodInfo>> MethodCache =
            new Dictionary<Type, List<MethodInfo>>();

        private static Dictionary<Type, Dictionary<string, PropertyInfo>> PropertyCache =
        ...
        {
            ExtendHooks.Add(new KeyValuePair<Type, Func<dynamic, dynamic>>(typeof(T), (i) =>
            {
                extension(i);
                return null;
            }));
        ...
        private void AttachDtoValues(object dto)
        {
            ...
            else if (dto is string || dto.GetType().IsValueType)
                dto = new { Value = dto };

            else if (dto is IEnumerable && !dto.CanConvertToPrototype())
                dto = new { Items = dto };

            if (dto is Prototype)
                Prototype = dto;
            else
                Prototype = dto.ToPrototype();
        }
        ...
        private void ApplyExtensions()
        {
            ...
            foreach (var include in includes)
            {
                ...
                (result.ToPrototype() as IDictionary<string, object>)
                    .ToList()
                    .ForEach(s => SetMember(s.Key, s.Value));
            }
        }
        ...
        private void AddDynamicMember(MethodInfo method)
        {
            ...
            if (IsDynamicFunction(method, parameters))
                TrySetMember(method.Name, DynamicFunctionFor(method));

            else if (IsDynamicFunctionWithParam(method, parameters))
                TrySetMember(method.Name, DynamicFunctionWithParamFor(method));

            else if (IsDynamicMethod(method, parameters))
                TrySetMember(method.Name, DynamicMethodFor(method));

            else if (IsDynamicMethodWithParam(method, parameters))
                TrySetMember(method.Name, DynamicFunctionWithParamFor(method));
        }
        ...
        {
            if (method.ReturnType != typeof(object) && method.ReturnType != typeof(IEnumerable<dynamic>))
                return false;
        ...
        public IEnumerable<MethodInfo> DynamicDelegates(Type type)
        {
            ...
            lock (typeof(Prism))
            {
                ...
                if (type == typeof(Prism))
                    delegates.RemoveAll(s => s.Name != "SetMembers");
        ...
        public bool TrySetMember(string property, object value, bool suppress = false)
        {
            ...
            else
            {
                ...
                if (!suppress)
                {
                    ...
                    foreach (var hook in hooks)
                        hook.Value(new Prism(new { Name = property, Instance = this }));
                }
            }
        ...
        {
            if (o is Prism && this.Prototype == (o as Prism).Prototype)
                return true;
        ...
        public override bool TryInvokeMember(InvokeMemberBinder binder, object[] args, out object result)
        {
            ...
            if (!RespondsTo(binder.Name))
                return MethodMissing(binder, args, ref result);

            var member = GetMember(binder.Name);

            if (!DynamicFunction(member))
        ...
        public virtual dynamic InvokeDynamicFunction(dynamic member, object[] args, string[] argNames)
        {
            ...
            if (member is DynamicFunctionWithParam)
            {
                ...
                if (argNames.Any())
                    argsToInvokeWith = GetNamedArgs(args, argNames);
                else if (args.Count() > 1)
                    argsToInvokeWith = args;
        ...
        dynamic ToDynamicParam(object[] args, string[] argNames)
        {
            if (AllParametersAreNamed(args, argNames))
            {
                ...
                for (int i = 0; i < args.Count(); i++)
                    prototype.Add(argNames[i], args[i]);
        ...
        public virtual dynamic Select(params string[] args)
        {
            ...
            {
                if (RespondsTo(s))
                    prototype.Add(s, GetMember(s));
            });

            return new Prism(prototype);
        }
        ...
        public virtual dynamic Exclude(params string[] args)
        {
            ...
            prototype.ForEach(s =>
            {
                if (!args.Contains(s.Key.ToLower()))
                    dictionary.Add(s.Key, GetMember(s.Key));
            });

            return new Prism(dictionary);
        }
    ...
    /// <summary>
    /// Local TypeExtensions so calls from Prism resolve here
    /// instead of ambiguous framework TypeExtensions.
    /// </summary>
    public static class TypeExtensions
    {
        public static bool IsOfType<T>(object instance)
        {
            return instance is T;
        }

        public static bool IsOfKind<T>(object instance)
        {
            // For our usage, "kind" is the same as "type".
            return instance is T;
        }
    }
}