...
{
    // http://www.dhpoware.com/demos/xnaFirstPersonCamera.html (adapted)
    public abstract class BaseCamera : IGameComponent
    {
        ...
        protected static readonly Vector3 WORLD_X_AXIS = Vector3.UnitX;
        protected static readonly Vector3 WORLD_Y_AXIS = Vector3.UnitY;
        protected static readonly Vector3 WORLD_Z_AXIS = Vector3.UnitZ;
        ...
        protected readonly Vector2[] _mouseMovement = { Vector2.Zero, Vector2.Zero };
        protected readonly Vector2[] _mouseSmoothingCache = new Vector2[MOUSE_SMOOTHING_CACHE_SIZE];
        ...
        public Vector3 Acceleration { get; set; } =
            new Vector3(DEFAULT_ACCELERATION_X, DEFAULT_ACCELERATION_Y, DEFAULT_ACCELERATION_Z);
        ...
        /// <summary>
        /// World-space camera target (used for helpers like FocusOnBounds).
        /// Setting this will re-orient the camera to look at the new target.
        /// </summary>
        ...
        public Vector3 VelocityStandard { get; set; } =
            new Vector3(DEFAULT_VELOCITY_X, DEFAULT_VELOCITY_Y, DEFAULT_VELOCITY_Z);

        public Vector3 VelocityFast { get; set; } =
            new Vector3(DEFAULT_VELOCITY_X, DEFAULT_VELOCITY_Y, DEFAULT_VELOCITY_Z) * DEFAULT_FAST_MULTIPLIER;

        protected Vector3 _viewDir = Vector3.Forward;
        public Vector3 ViewDirection => _viewDir;

        /// <summary>Convenience forward direction (alias of ViewDirection).</summary>
        public Vector3 Forward => ViewDirection;

        /// <summary>Camera right vector.</summary>
        public Vector3 Right => _xAxis;

        /// <summary>Camera up vector.</summary>
        public Vector3 Up => _yAxis;
        ...
        public void LookAt(Vector3 eye, Vector3 target, Vector3 up)
        {
            ...
            _accumPitchDegrees =
                MathHelper.ToDegrees((float)Math.Asin(_viewMatrix.M23));
            _accumHeadingDegrees =
                MathHelper.ToDegrees((float)Math.Atan2(_viewMatrix.M13, _viewMatrix.M33));

            Quaternion.CreateFromRotationMatrix(ref _viewMatrix, out _orientation);
        }

        /// <summary>
        /// Moves the camera by dx world units to the left/right; dy up/down; dz forward/back.
        /// </summary>
        public void Move(float dx, float dy, float dz)
        {
            // "Forward" independent of pitch, so strafing doesn't slow when looking up/down.
        ...
        public void Rotate(float headingDegrees, float pitchDegrees)
        {
            ...
            if (heading != 0.0f)
            {
                var worldY = WORLD_Y_AXIS;
                Quaternion.CreateFromAxisAngle(ref worldY, heading, out var rotation);
            ...
            if (pitch != 0.0f)
            {
                var worldX = WORLD_X_AXIS;
                Quaternion.CreateFromAxisAngle(ref worldX, pitch, out var rotation);
        ...
        public Ray GetMouseRay()
        {
            ...
            nearPoint = GraphicsDevice.Viewport.Unproject(
                nearPoint,
                ProjectionMatrix,
                ViewMatrix,
                Matrix.Identity);
            farPoint = GraphicsDevice.Viewport.Unproject(
                farPoint,
                ProjectionMatrix,
                ViewMatrix,
                Matrix.Identity);
        ...
        /// <summary>Top-down view helper.</summary>
        public virtual void LookFromTop()
        {
            var center = Target;
            _eye = center + new Vector3(0, 50, 0);
            LookAt(center);
        }

        /// <summary>Front view helper.</summary>
        public virtual void LookFromFront()
        {
            var center = Target;
            _eye = center + new Vector3(0, 10, 50);
            LookAt(center);
        }

        /// <summary>Side view helper.</summary>
        public virtual void LookFromSide()
        {
            var center = Target;
            _eye = center + new Vector3(50, 10, 0);
            LookAt(center);
        }

        #endregion

        #region Abstract platform hooks
        ...
        #region Mouse + movement helpers

        protected void PerformMouseFiltering(Vector2 mouse)
        {
            for (var i = _mouseSmoothingCache.Length - 1; i > 0; --i)
            {
                _mouseSmoothingCache[i] = _mouseSmoothingCache[i - 1];
            }

            _mouseSmoothingCache[0] = mouse;
        ...
        {
            _mouseMovement[_mouseIndex] = mouse;
            ...
            _mouseIndex ^= 1;
            _mouseMovement[_mouseIndex] = Vector2.Zero;
        }
        ...
        protected void UpdateMouse(MouseState mouseState)
        {
            ...
            if (_currentMouseState.RightButton == ButtonState.Pressed)
            {
                ...
                var delta = new Vector2(
                    center.X - _currentMouseState.X,
                    center.Y - _currentMouseState.Y);
        ...
        protected void UpdatePosition(ref Vector3 direction, float elapsedTimeSec)
        {
            ...
            {
                var displacement =
                    _currentVelocity * elapsedTimeSec +
