...
{
    public class FontRendererDefault : IFontRenderer, IDisposable
    {
        ...
        public FontRendererDefault(IEnumerable<FontFile> fontFiles)
        {
            ...
            // sort biggest ? smallest so [0] is max
        ...
        {
            // Pick a font whose scale fits inside 'borders'
            var maxFont = Fonts[0];
        ...
        public void DrawText(SpriteBatch spriteBatch, string text, Rectangle borders, Color color, bool centerHeight = true)
        {
            ...
            var dx = borders.X;
            var dy = centerHeight
                ? borders.Center.Y - (int)(size.Y * 0.5f)
                : borders.Y;
        ...
        {
            // Left commented-out: no unreachable code in here.
            /*
